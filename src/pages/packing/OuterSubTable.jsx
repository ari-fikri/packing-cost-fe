// src/components/packing/OuterSubTable.jsx
 ...
 *  - outerArr: array of length 10 (price or per-material values). If shorter, missing values default to 0.
 *
 * Behavior:
 *  - Shows a title row with a chevron + "OUTER" (chevron toggles expand/collapse)
 ...
 *      - one data row using part.partNo, qty=1, price from outerArr, sum = price * qty
 */
      ...
      <table className="table table-sm table-bordered mb-1 w-100 subtable-small">
          ...
          <tr style={{ backgroundColor: "#d0d0d0" }}>
            <th colSpan={40} className="text-left" style={{ verticalAlign: "middle" }}>
              <button
                ...
                title={expanded ? "Collapse OUTER" : "Expand OUTER"}
              >
        ...
        {expanded && (
          <tbody>
            <tr>
              {Array.from({ length: 10 }).map((_, m) => {
                const price = Number(outerArr[m] || 0);
                const qty = 1;
                const sum = price * qty;
                return (
